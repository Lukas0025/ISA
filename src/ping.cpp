...
namespace ping {
    ...
    bool send(ping_packet_t packet, addresses::addr_t addr) {
        if (addr.type == IPV4_TYPE) {
            return ipv4send(packet, addr.v4);
        } else if (addr.type == IPV6_TYPE) {
            return ipv6send(packet, addr.v6);
        }
    }

    bool ipv4send(ping_packet_t packet, in_addr addr) {

    }

    bool ipv6send(ping_packet_t packet, in_6addr addr) {
